...
class Division extends Model
{
    ...
    public function calculateStandings(): array
    {
        ...
        // ---- Per-fixture rows for each contestant in this division (home + away) ----
        $resultsSubquery = DB::table('results')
            ...
            ->join('contestants as hc', function ($join) {
                $join->on('hc.id', '=', 'results.home_contestant_id')
                    ->whereNull('hc.deleted_at');
            })
            ->join('contestants as ac', function ($join) {
                $join->on('ac.id', '=', 'results.away_contestant_id')
        ...
        // ---- Aggregate per (current) contestant in this division ----
        $subquery = DB::table('contestants')
            ...
            ->join('members', 'contestants.member_id', '=', 'members.id')
            ...
            ->selectRaw("
                ...
                /* use SIGNED casts to avoid BIGINT UNSIGNED underflow on differences */
        ...
        // Window rank (it’s one division, but we keep the same rule)
        ...
        // ---- Build division header (even if no rows yet) ----
        if ($rows->isEmpty()) {
            // Fallback metadata if division has no contestants/results yet
        ...
        // Use first row for division/tier metadata
        $first = $rows->first();
        ...
        return [
            ...
            'standings' => $rows->map(function ($row) {
                return [
                    ...
                    'club_member_id' => (int) $row->club_member_id,
