...
<div class="space-y-main">
    <flux:breadcrumbs>
        ...
        <flux:breadcrumbs.item href="{{ route('club.admin.leagues.sessions.show', [$club, $league, $session]) }}" wire:navigate>
            {{ $session->active_period }}
        </flux:breadcrumbs.item>
        <flux:breadcrumbs.item>{{ __("Competitors") }}</flux:breadcrumbs.item>
    </flux:breadcrumbs>

    <x-containers.club-admin-session :$club :$league :$session :showNavbar="true">
        <x-headings.page-heading>Competitors</x-headings.page-heading>
