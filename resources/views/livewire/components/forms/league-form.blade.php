        ...
        @else
            <div class="flex-1 sm:flex sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                <div class="flex items-center gap-2 h-10">
                    <div>{{ $league->name }}</div>
                    @if ($league->trashed())
                        <flux:badge color="red">{{ __('Archived') }}</flux:badge>
                    @endif
                </div>
                @if (! $league->trashed())
                    @if ($league->latestSession)
                        <flux:button href="{{ route('club.admin.leagues.sessions.show', [$club, $league, 'session' => $league->latestSession]) }}" variant="filled" icon:trailing="square-arrow-out-up-right" wire:navigate>{{ __('Latest Session') }}</flux:button>
                    @else
                        <flux:button href="{{ route('club.admin.leagues.sessions.create', [$club, $league]) }}" icon="plus" variant="primary" wire:navigate>{{ __('Session') }}</flux:button>
                    @endif
                @endif
            </div>
        @endif
    ...
    <x-containers.club-admin-form>
        <form wire:submit="save" class="space-y-6">
            ...
            <div class="flex gap-2">
                ...
                @if ($isEdit)
                    @if ($league->trashed())
                        <flux:button wire:click="restore" variant="primary" color="green">Restore</flux:button>
                    @else
                        <flux:button wire:click="archive" variant="primary" color="amber">Archive</flux:button>
                    @endif
                                    ...
                                    <flux:table class="border">
                                        <flux:table.rows>
                                            <flux:table.row>
                                                ...
                                                <flux:table.cell class="font-medium text-zinc-900 whitespace-normal">
                                                    {{ $form->name }}
                                                </flux:table.cell>
                                            </flux:table.row>
                                            <flux:table.row>
                                                ...
                                                <flux:table.cell class="flex items-center gap-1 font-medium text-zinc-900">
