...
<div class="space-y-6">
    ...
    @else
        <flux:radio.group wire:model.live="searchLeagueStatus" variant="cards" :indicator="false" class="flex gap-4">
            <x-clubs.backend.indexs.radio value="{{ ModelStatus::Active }}" heading="Active" count="{{ $this->league_active_count }}" />
            <x-clubs.backend.indexs.radio value="{{ ModelStatus::Trashed }}" heading="Archived" count="{{ $this->league_trashed_count }}" />
            <x-clubs.backend.indexs.radio value="{{ ModelStatus::All }}" heading="All" count="{{ $this->league_all_count }}" />
        </flux:radio.group>
                ...
                @if ($this->leagues->total() > 0)
                    <flux:table.rows>
                        @foreach ($this->leagues as $league)
                            <livewire:components.tables.rows.club-admin-leagues-row :$club :$league :key="$league->id" />
                        @endforeach
                    </flux:table.rows>
                @else
