...
new class extends Component
{
        ...
        foreach ($this->structure as $tier) {
            foreach ($tier['divisions'] as $division) {
                foreach ($division['contestants'] as $contestant) {
                    if (isset($contestant['member_id'])) {
                        $usedMemberIds[] = $contestant['member_id'];
                    }
                }
            }
        }
    ...
    public function saveTierName(int $tierId, string $name)
    {
        $name = trim($name);
        ...
        $this->editingTierId = -1;
        $this->structure = $this->buildStructure();

        Flux::toast(
            variant: 'success',
            text: 'Tier name updated.'
        );
    }
    ...
    public function filledSlotCount(): int
    {
        ...
        foreach ($this->structure as $tier) {
            ...
            foreach ($tier['divisions'] as $division) {
                ...
                foreach ($division['contestants'] as $slot) {
                    if (!empty($slot['member_id'])) { // why: treat any truthy member_id as filled
                        $filled++;
                    }
                }
            }
    ...
    public function sortSlot(int $rank, int $targetIndex, int $targetTierIndex, int $targetDivisionIndex): void
    {
        ...
        $slot             = $hit['slot'] ?? [];
        $movingContestantId = isset($slot['id']) ? (int) $slot['id'] : null;
    ...
    public function relegate($inc, $divisionId)
    {
        $division = Division::findOrFail($divisionId);

        $relegate_count = $division->relegate_count + $inc;
        $promote_count = $division->contestant_count - $relegate_count;
        ...
        DB::transaction(function () use ($division, $promote_count, $relegate_count) {
            ...
            if ($division->promote_count > $promote_count) {
                $division->update([
                    'promote_count' => $promote_count,
                ]);
            }
            ...
            >
                <div class="space-y-6">
                    <div class="sm:flex sm:items-center sm:justify-between sm:flex-row-reverse gap-2 space-y-6 sm:space-y-0">
                        ...
                        <div
                            x-data="{
                                localTierName: {{ json_encode($tier['name']) }},
                                localOldTierName: {{ json_encode($tier['old_name']) }},
                                tierId: {{ $tier['id'] }},
                            }"
                        ...
                        >
                            ...
                            <div x-show="$wire.editingTierId !== tierId" class="flex items-center gap-1 h-10">
                                <flux:heading variant="strong" size="lg">{{ $tier['name'] }}</flux:heading>
                                <div class="flex items-center">
                                    <flux:button
                                        @click="$wire.editingTierId = tierId;"
                                        icon="square-pen"
                                        variant="subtle"
                                        size="xs"
                                    />
                            ...
                            <flux:input.group x-show="$wire.editingTierId === tierId" @click.outside="$wire.editingTierId = -1; localTierName = localOldTierName" class="!w-fit">
                                <flux:input x-model="localTierName" class="max-w-64" />
                                <flux:button
                                    x-on:click="$wire.saveTierName({{ $tier['id'] }}, localTierName)"
                                    x-bind:disabled="!(localTierName && localTierName.trim().length)"
                                    icon="check"
                                    variant="primary"
                                    square
                                    class="shrink-0"
                                            ...
                                            >
                                                @foreach (range(0, $division['contestant_count'] - 1) as $contestantIndex)
                                                    ...
                                                    <div
                                                        ...
                                                        @class([
                                                            '!bg-white' => array_key_exists('member_id', $slot),
                                                            '!bg-red-50 !border-red-200' => !array_key_exists('member_id', $slot),
                                                            '!border rounded-md shadow-xs flex items-center justify-between gap-2 p-1'
                                                        ])
                                                        wire:key="{{ $contestantIndex }}"
                                                    >
                                                        <div class="flex-1 flex items-center gap-1">
                                                            <flux:button size="xs" icon="bars-3" variant="subtle" class="!shrink-0" x-sort:handle />
                                                            @if ($slot['content'])
                                                                <div
                                                                    @class([
                                                                ...
                                                                >
                                                                    ...
                                                                    <div class="text-gray-500 text-xs hidden sm:block">M{{ $slot['club_member_id'] }}</div>
                                                                    <div class="text-xs hidden sm:block">#{{ $slot['seed'] }}</div>
                                                                    @if ($slot['deleted_at'])
                                                                        <flux:tooltip>
                                                                            <flux:icon.no-symbol
                                                                                variant="micro"
                                                                                class="text-red-500"
                                                                            />
                                                                            <flux:tooltip.content class="tooltip">This member cannot submit results because they have been archived.</flux:tooltip.content>
                                                                        </flux:tooltip>
                                                                    @endif
                                                                </div>
                                                            @endif
                                                            @if (array_key_exists('id', $slot))
                                                                <flux:tooltip>
                                                                    <flux:button
                                                                        wire:click="deleteContestant({{ $slot['id'] }})"
                                                                        icon="eraser"
                                                                        icon:variant="outline"
                                                                        variant="subtle"
                                                                        size="xs"
                                                                        ...
                                                                        <form wire:submit="addContestant({{ $division['id'] }}, {{ $contestantIndex }})">
                                                                            <x-modals.content>
                                                                                ...
                                                                                @elseif ($this->entrantCount === 0)
                                                                                    <flux:text>
                                                                                        Please add some entrants.
                                                                                    </flux:text>
                                                                                @else
                                                                                    <flux:text>
                                                                                        All entrants have been allocated. To include additional participants, add more entrants.
                                                                                    </flux:text>
                                                                                @endif
                                                                            </x-modals.content>
                                                                        </form>
                                                        ...
                                                        <div class="flex items-center gap-1">
                                                            <div class="flex items-center gap-2.5 text-xs text-gray-500">
                                                                @if (array_key_exists('id', $slot))
                                                                    @if (array_key_exists('member_id', $slot) && $slot['seed'] !== $slot['rank'])
                                                                        <flux:tooltip>
                                                                            <flux:icon.exclamation-circle
                                                                                variant="micro"
                                                                                class="text-amber-500"
                                                                            />
                                                                            <flux:tooltip.content class="tooltip">#{{ $slot['seed'] }} Seed in Slot {{ $slot['rank'] }}</flux:tooltip.content>
                                                                        </flux:tooltip>
                                                                    @else
                                                                        <flux:tooltip>
                                                                            <flux:icon.check
