        ...
        DB::transaction(function () {
            ...
            $push = function ($tier, $destDivision, $row) use (&$overall, &$overallRank) {
                $overall[] = [
                    ...
                    'member_id'      => data_get($row, 'member_id'),
            ...
            foreach ($overall as $contestant) {
                $this->session->contestants()
                    ->where('member_id', $contestant['member_id'])
                    ->update([
...
<div class="relative space-y-8">
    ...
    <div class="flex items-center justify-between min-h-10">
        <flux:heading size="xl" variant="strong">{{ __("Tables") }}</flux:heading>
        @if (is_null($session->processed_at))
            <livewire:components.buttons.destroy-tables :$club :$league :$session />
        @elseif ($isLatestSession)
            <flux:button
    ...
    <div class="space-y-12">
        <div class="space-y-6 flex flex-col-reverse md:space-y-0 md:grid md:grid-cols-5 gap-6">
                                        ...
                                        <x-modals.content>
                                            <x-slot:heading>{{ __('Make Session Live') }}</x-slot:heading>
                                                <flux:table class="border sm:min-w-3xs mb-2">
                                                    ...
                                                    <flux:table.rows>
                                                        <flux:table.row>
                                                            <flux:table.cell class="!text-zinc-900 !font-medium">Winning</flux:table.cell>
                                                            <flux:table.cell align="center" class="!text-zinc-900 !font-medium">{{ $session->pts_win }}</flux:table.cell>
                                                        </flux:table.row>
                                                        <flux:table.row>
                                                            <flux:table.cell class="!text-zinc-900 !font-medium">Drawing</flux:table.cell>
                                                            <flux:table.cell align="center" class="!text-zinc-900 !font-medium">{{ $session->pts_draw }}</flux:table.cell>
                                                        </flux:table.row>
                                                        <flux:table.row>
                                                            <flux:table.cell class="!text-zinc-900 !font-medium">Each {{ $tallyUnit }} Won</flux:table.cell>
                                                            <flux:table.cell align="center" class="!text-zinc-900 !font-medium">{{ $session->pts_per_set }}</flux:table.cell>
                                                        </flux:table.row>
                                                        <flux:table.row>
                                                            <flux:table.cell class="!text-zinc-900 !font-medium">Turning Up</flux:table.cell>
                                                            <flux:table.cell align="center" class="!text-zinc-900 !font-medium">{{ $session->pts_play }}</flux:table.cell>
                                                        </flux:table.row>
                                                    </flux:table.rows>
                                                </flux:table>
            ...
            <div class="relative" wire:key="{{ $tier['id'] }}">
                <div class="space-y-9">
                    ...
                    @foreach ($tier['divisions'] as $division)
                        <div class="relative space-y-9 border-t pt-8" wire:key="{{ $division['id'] }}">
                            @if ($loop->iteration === 1)
                                <div class="absolute left-0 right-0 -top-5.25">
                                    <div class="flex flex-col items-center">
                                        <div class="flex items-center gap-1 bg-white px-4 py-2 border rounded-lg">
                                            <flux:heading>Tier {{ $loop->parent->iteration }}</flux:heading>
                                            @if (is_null($session->processed_at))
                                                <flux:button
                                                    x-on:click="$js.openModal({{ $tier['id'] }}, '{{ $tier['name'] }}');$flux.modal('update-tier-name-{{ $tier['id'] }}').show()"
                                                    variant="subtle"
                                                    size="xs"
                                                    icon="pencil-square"
                                            ...
                                            @teleport('body')
                                                <flux:modal name="update-tier-name-{{ $tier['id'] }}" class="modal">
                                                    <form wire:submit="updateTierName({{ $tier['id'] }})">
                                                        <x-modals.content>
                            ...
                            <div class="flex items-center justify-between">
                                <flux:heading size="xl">{{ $division['name'] }}</flux:heading>
                            ...
                            <flux:table class="border">
                                ...
                                <flux:table.rows class="!divide-y-0">
                                    ...
                                    @php $standings = $division['standings']; @endphp
                                    @foreach ($standings as $i => $contestant)
                                        ...
                                        <flux:table.row>
                                            ...
                                            <flux:table.cell class="flex items-center justify-between gap-4">
