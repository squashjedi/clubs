...
new class extends Component
{
    ...
    {
        $matrix = [];
        $ids = $this->division->contestants()->withTrashed()->get()->pluck('id')->all();
        $empty = [
            'original_for'   => null,
            'for'            => null,
            'points'         => 0,
        ...
        foreach ($this->division->results as $res) {
            ...
            $matrix[$h][$a] = array_merge($empty, $cellData, [
                ...
                'original_attended'     => (bool) $res->home_attended ? false : true,
                'attended'     => (bool) $res->home_attended ? false : true,
            ]);
            ...
            $matrix[$a][$h] = array_merge($empty, $cellData, [
                ...
                'original_attended'     => (bool) $res->away_attended ? false : true,
                'attended'     => (bool) $res->away_attended ? false : true,
            ]);
        }
...
<div class="relative space-y-6">
    ...
    <div class="flex items-center justify-between mt-6">
        <div class="sm:flex sm:items-center gap-4">
            ...
            @if ($tier->index > 0 && $division->promote_count > 0)
            ...
            @endif
            @if ($tier->index + 1 < $tierCount && $division->relegate_count > 0)
                <div class="flex items-center gap-0.5">
                ...
                <flux:table.row wire:key="{{ $row->id }}">
                    <flux:table.cell class="h-28 flex items-center justify-between gap-4">
                        <div class="flex-1 flex items-center justify-between gap-2">
                            <div
                                @class([
                                    'opacity-50' => $row->deleted_at
                                ])
                            >
                                <flux:heading>
                                    <x-generic.member :$club :member="$row->member" />
                                </flux:heading>
                            </div>
                        </div>
                            ...
                            @if ($row->id !== $col->id)
                                <div class="absolute inset-0 flex items-center justify-center">
                                    <div
                                        @class([
                                            'flex flex-col items-center',
                                            'opacity-50' => $col->deleted_at || $row->deleted_at
                                        ])
                                    >
                                                ...
                                                <form wire:submit="save({{ $row->id }}, {{ $col->id }})" class="space-y-6">
                                                    <x-modals.content>
                                                        ...
                                                        <div class="space-y-2 mt-6">
                                                            ...
                                                            <div class="flex flex-col items-center">
                                                                <flux:text class="text-center w-80 !text-xs">Tick a player's checkbox if didn't turn up so their opponent can claim the match.</flux:text>
                                                            </div>
                                                        </div>
