...
new class extends Component
{
    ...
    public EloquentCollection $entrants;
    ...
    {
        return $this->query()->orderBy('index')->get();
    }
    ...
    public function remove(int $id): void
    {
        DB::transaction(function () use ($id) {
            $entrant = $this->session->entrants()->findOrFail($id);
            ...
            $allAfterDelete = $this->session->entrants()->orderBy('index')->get();

            foreach ($allAfterDelete as $newIndex => $remainingEntrant) {
                $remainingEntrant->update(['index' => $newIndex]);
            }
                ...
                <div x-data="{ showErrors: true }" class="space-y-2">
                    <div x-sort="$wire.sort($item, $position)" class="space-y-1">
                        @foreach ($entrants as $entrant)
                            ...
                            >
                                <x-generic.entrant-tile
                                    :$entrant
                                    :hasPreviousSession="$this->hasPreviousSession"
                                    :$isAllocated
                                    :originalMemberIds="$this->originalMemberIds"
                                    :editable="true"
                                />
                            </div>
                        @endforeach
