<?php
// database/migrations/2025_10_25_013000_add_away_contestant_id_after_home_to_results.php
...
return new class extends Migration {
    ...
    private const NEW_COL = 'away_contestant_id';
    private const AFTER_COL = 'home_contestant_id';
    private const FIXTURE_CONTESTANTS = 'fixture_contestants';
    private const CONTESTANTS = 'contestants';
    private const FK_NAME = 'results_away_contestant_id_foreign';

    public function up(): void
    {
        // Add as NULLable for safe backfill (placed after home_contestant_id).
        ...
        // Backfill from fixture_contestants.opp_contestant_id via results.fixture_contestant_id.
        DB::statement(sprintf(
            'UPDATE `%1$s` r
               JOIN `%2$s` fc ON fc.`id` = r.`fixture_contestant_id`
               SET  r.`%3$s`  = fc.`opp_contestant_id`
             WHERE r.`%3$s` IS NULL',
            self::TABLE, self::FIXTURE_CONTESTANTS, self::NEW_COL
        ));

        // Drop existing FK on the column if present (avoid 1091/name mismatch).
        ...
        // Ensure no NULLs remain, then enforce NOT NULL without doctrine/dbal.
