<?php
// database/migrations/2025_10_25_041500_drop_competitor_id_from_results.php
...
return new class extends Migration {
    ...
    private const INDEX_NAME = 'results_competitor_id_index';
    private const FK_NAME    = 'results_competitor_id_foreign';
    ...
    public function up(): void
    {
        ...
        // 1) Drop FK by discovered name (prevents 1091 errors)
        ...
        // 2) Drop any secondary indexes on the column
    ...
    {
        // Recreate the column (nullable; data cannot be restored)
        Schema::table(self::TABLE, function (Blueprint $table) {
            if (!Schema::hasColumn(self::TABLE, self::COL)) {
                // Adjust placement if your schema differs
                $table->unsignedBigInteger(self::COL)->nullable()->after('division_id');
                $table->index(self::COL, self::INDEX_NAME);
            }
        });

        // Recreate FK with predictable name
        Schema::table(self::TABLE, function (Blueprint $table) {
            try {
